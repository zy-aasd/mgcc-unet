...
class launch_seg(nn.Module):
    def __init__(self, in_ch=3, start_ch=48, num_classes=9, img_size=224, granular_sizes=[224,112,56], up_factor=[1,2,4], loops=[1,2,3,4]):
        ...
        self.decoder = Decoder(start_ch=start_ch, out_ch=num_classes)
        # Decoder(start_ch=start_ch, out_ch=num_classes)
        # DecoderGB(start_ch=start_ch, out_ch=num_classes, granular_sizes=self.granular_sizes, up_factor=up_factor, loops=loops)
