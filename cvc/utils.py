...
def compute_metric(pred, label):
    ...
    true_labels = label.astype(np.int64)
    # confusion matrix
    TP = np.sum((predicted_labels == 1) & (true_labels == 1))  # True positive
    TN = np.sum((predicted_labels == 0) & (true_labels == 0))  # True negative
    FP = np.sum((predicted_labels == 1) & (true_labels == 0))  # False positive
    FN = np.sum((predicted_labels == 0) & (true_labels == 1))  # False negative
    # IoU, Rec, Pre
    IoU = TP / (TP + FP + FN)
    Rec = TP / (TP + FN)
    Pre = TP / (TP + FP)
